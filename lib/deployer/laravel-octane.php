...
// Graceful Octane worker reload after symlink — only if laravel/octane is present.
task('octane:reload', function () {
    run('[ ! -L {{deploy_path}}/current ] || [ ! -f {{deploy_path}}/current/vendor/laravel/octane/composer.json ] || {{bin/php}} {{deploy_path}}/current/artisan octane:reload 2>/dev/null || true');
});
